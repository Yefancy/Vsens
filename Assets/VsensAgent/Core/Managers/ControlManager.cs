...
namespace VsensAgent
{
    public class ControlManager : MonoBehaviour
    {
        ...
        private void HandleControlBatch(ControlObject[] controlActions)
        {
            ...
            var captureRequest = BuildAvatarValidationCaptureRequest(controlActions);
            if (captureRequest != null)
            {
                ScoreAndRetryAvatarValidationViews(captureRequest);
            }
        }
        ...
        private AvatarValidationCaptureRequest BuildAvatarValidationCaptureRequest(ControlObject[] controlActions)
        {
            if (controlActions == null || controlActions.Length == 0)
            {
                return null;
            ...
            foreach (var ctrl in controlActions)
            {
                if (ctrl == null || !IsAvatarAction(ctrl.action))
                {
                    continue;
                }

                if (!ParseBoolParameter(ctrl.parameters, "capture_validation_views", false))
                {
                    continue;
                }

                string targetObjectId = ParseStringParameter(ctrl.parameters, "validation_target_object_id", string.Empty);
                string targetAlias = ParseStringParameter(ctrl.parameters, "validation_target_alias", string.Empty);
                if (string.IsNullOrWhiteSpace(targetObjectId) && string.IsNullOrWhiteSpace(targetAlias))
                {
                    continue;
                }

                return new AvatarValidationCaptureRequest
                {
                    AvatarId = string.IsNullOrWhiteSpace(ctrl.target) ? ParseStringParameter(ctrl.parameters, "avatar_id", "avatar_main") : ctrl.target,
                    ValidatorContext = ParseStringParameter(ctrl.parameters, "validator_context", "avatar_validation"),
                    TargetObjectId = targetObjectId,
                    TargetAlias = targetAlias,
                    TaskHint = ParseStringParameter(ctrl.parameters, "validation_task_hint", string.Empty),
                    CurrentCandidateId = ParseStringParameter(ctrl.parameters, "validation_candidate_id", string.Empty),
                    Candidates = ParseValidationCandidates(ctrl.parameters.ContainsKey("validation_candidates")
                        ? ctrl.parameters["validation_candidates"]
                        : null),
                    PlaybackSpeed = ParseFloatParameter(ctrl.parameters, "speed", 1f),
                    PlaybackLoop = ParseBoolParameter(ctrl.parameters, "loop", true),
                    ReplayMotionAfterRetry = ctrl.action == "play_avatar_motion",
                };
            }
        ...
        private void ScoreAndRetryAvatarValidationViews(AvatarValidationCaptureRequest request)
        {
            ResolveSceneRegistry();
            ResolveAvatarRuntimeManager();
            if (sceneRegistry == null || avatarRuntimeManager == null)
            {
                return;
            }

            AvatarValidationDebugState.RegisterCandidateContext(
                request.ValidatorContext,
                request.Candidates.Select(candidate => new AvatarValidationCandidateDebugInfo
                {
                    CandidateId = candidate.CandidateId,
                    Position = candidate.Position ?? Vector3.zero,
                    RotationEuler = candidate.Rotation ?? Vector3.zero
                }),
                request.CurrentCandidateId);

            var validationService = new SceneValidationService(sceneRegistry, avatarRuntimeManager);
            int attemptIndex = Mathf.Max(0, request.Candidates.FindIndex(candidate => candidate.CandidateId == request.CurrentCandidateId));
            int maxAttempts = request.Candidates.Count == 0 ? 1 : Mathf.Min(request.Candidates.Count, 3);

            try
            {
                for (int attempts = 0; attempts < maxAttempts; attempts++)
                {
                    AvatarValidationDebugState.UpdateCurrentCandidate(request.ValidatorContext, request.CurrentCandidateId);
                    var response = JObject.FromObject(validationService.ScoreValidationViews(
                        request.ValidatorContext,
                        request.AvatarId,
                    ...
                    var score = response.ToObject<ValidationViewScoreEnvelope>() ?? new ValidationViewScoreEnvelope();
                    foreach (var artifact in score.artifacts)
                    {
                        Debug.Log($"[ControlManager] Validation capture saved: {artifact.file_path}");
                    }
                    ...
                    if (score.recommended_action != "retry_next_candidate")
                    {
                        return;
                    }

                    int nextIndex = attemptIndex + 1;
                    if (request.Candidates == null || nextIndex >= request.Candidates.Count)
                    {
                        return;
                    }

                    var nextCandidate = request.Candidates[nextIndex];
                    if (!avatarRuntimeManager.TrySetAvatarTransform(
                            request.AvatarId,
                            nextCandidate.Position,
                            nextCandidate.Rotation,
                            null,
                            out var errorMessage))
                    {
                        Debug.LogWarning($"[ControlManager] Visual retry failed for candidate '{nextCandidate.CandidateId}': {errorMessage}");
                        return;
                    }

                    attemptIndex = nextIndex;
                    request.CurrentCandidateId = nextCandidate.CandidateId;
                    RegisterMutation("control.visual_retry", request.AvatarId, "set_avatar_transform");

                    if (request.ReplayMotionAfterRetry)
                    {
                        avatarRuntimeManager.TryStopAvatarMotion(request.AvatarId, out _);
                        avatarRuntimeManager.TryPlayAvatarMotion(request.AvatarId, request.PlaybackSpeed, request.PlaybackLoop, out _);
                    }
                }
            }
            finally
            {
                AvatarValidationDebugState.ClearContext(request.ValidatorContext);
            }
        }

        private sealed class AvatarValidationCaptureRequest
        {
            public string AvatarId;
            public string ValidatorContext;
            public string TargetObjectId;
            public string TargetAlias;
            public string TaskHint;
            public string CurrentCandidateId;
            public List<AvatarValidationCandidate> Candidates = new List<AvatarValidationCandidate>();
            public float PlaybackSpeed = 1f;
            public bool PlaybackLoop = true;
            public bool ReplayMotionAfterRetry;
        }

        private sealed class AvatarValidationCandidate
        {
        ...
        private sealed class ValidationViewScoreEnvelope
        {
            public bool passed;
        ...
        private List<AvatarValidationCandidate> ParseValidationCandidates(object value)
        {
            var result = new List<AvatarValidationCandidate>();
            if (value == null)
            {
                return result;
            }

            JArray tokens = value as JArray;
            if (tokens == null && value is object[] objectArray)
            {
                tokens = JArray.FromObject(objectArray);
            }
            ...
            if (tokens == null)
            {
                try
                {
                    tokens = JArray.FromObject(value);
                }
                catch
                {
                    return result;
            ...
            foreach (var token in tokens)
            {
                ...
                result.Add(new AvatarValidationCandidate
                {
                    CandidateId = obj.Value<string>("candidate_id") ?? string.Empty,
                    Position = ReadOptionalVector3(obj["position"]),
                    Rotation = ReadOptionalVector3(obj["rotation"]),
                });
            }
        ...
        private static Vector3? ReadOptionalVector3(JToken token)
        {
            if (token == null)
            {
                return null;
